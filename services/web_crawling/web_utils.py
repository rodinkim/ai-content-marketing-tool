...
def sanitize_filename(title: str, url: str, max_length: int = 150) -> str:
    """
    기사 제목과 URL 해시를 기반으로 안전하고 유니크한 파일명을 생성합니다.
    파일명은 '정화된_제목_URLHASH.txt' 형식이 됩니다.
    """
    # 1. 제목 정화: 특수문자 제거 및 공백을 언더스코어로 변경
    sanitized_title = re.sub(r'[^a-zA-Z0-9가-힣\s-]', '', title).replace(' ', '_').strip()
    # 2. URL 해시 생성 (MD5 해시의 앞 8자리)
    url_hash = hashlib.md5(url.encode('utf-8')).hexdigest()[:8]
    # 3. 최종 파일명 베이스 구성
    if not sanitized_title:
        # 제목이 없을 경우 URL 해시만으로 구성
        final_base = f"untitled_article_{url_hash}"
        logger.warning(f"제목 없이 파일명 생성: {url} -> {final_base}")
    else:
        # 제목이 너무 길 경우 적절히 자르고 해시를 붙임
        title_part_max_len = max_length - 13 # 해시+언더스코어+확장자 길이 고려
        if len(sanitized_title) > title_part_max_len:
            sanitized_title = sanitized_title[:title_part_max_len].rstrip('_')
        final_base = f"{sanitized_title}_{url_hash}"
    # 최종 파일명 길이 제한
    filename = final_base[:max_length].strip('_')
    # 모든 문자가 제거되어 파일명이 비어지는 경우 대비
    if not filename:
...
def decode_html_content(content: bytes, url: str) -> str:
    ...
    return content.decode('utf-8', errors='replace') # 최후의 수단
